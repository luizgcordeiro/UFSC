...
def determinante(A):
    ...
    T=escalonamento.triangulariza(np.matrix(A))
...
def matriz_inversivel(ordem,li=10):
    ...
    A=np.random.randint(low=-li,high=li,size=(ordem,ordem))
...
def transposta(A):
  '''Esta função transpõe uma matriz A'''
  shape=np.shape(A)
...
def inversa_por_escalonamento(A,tol=1.0e-14,verbose=False):
  '''Esta função calcula a inversa uma matriz A, caso exista, via escalonamento.;
  
  tol: tolerância numérica.
  verbose: True ou False. Determina se deve-se imprimir a explicação e detalhes dos cálculos.'''
  ...
  if m!=n:
    print('A matriz não é quadrada, e portanto não é inversível.')
    return
  elif abs(determinante(A))<tol:
    print('A matriz tem determinante nulo, e portanto não é inversível.')
    return
  else:
    return escalonamento.escalona(np.concatenate([A,np.eye(n)],axis=1),tol,verbose)[0][:,n:]
  #endif
